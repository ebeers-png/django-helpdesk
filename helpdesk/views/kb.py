...
def upload_attachment(request):
    ...
    if form.is_valid():
        ...
        if cf['file']:
            ...
            return JsonResponse({'uploaded': True, 'id': attach.id, 'url': create_presigned_url(settings.AWS_STORAGE_BUCKET_NAME, f'{attach.file.url}') if settings.USE_S3 is True else attach.file.url})
    else:
